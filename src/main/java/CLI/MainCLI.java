            ...
            if(command.equals("help")){
                ...
                while (commandTok.hasMoreTokens()) {
                    ...
                    if (command.equals("-sd")) {
                        startDate = Timestamp.valueOf(commandTok.nextToken().replace('_', ' '));

                    } else if (command.equals("-ed")) {
                        endDate = Timestamp.valueOf(commandTok.nextToken().replace('_', ' '));
                    } else if (command.equals("-c")) {
                        customer = session.get(CustomersEntity.class, commandTok.nextToken().replace('_', ' '));
                ...
                if (isValid && startDate != null && endDate != null && customer != null) {
                    Date curDate = new Date();
                    SimpleDateFormat format = new SimpleDateFormat("dd-MMM-yyy");
                    String date = format.format(curDate);
                    String invoiceName = CamelCase.toCamelCase(customer.getCompanyName());
                    invoiceName.concat("_");
                    invoiceName.concat(date);

                    Invoice.generateInvoice(invoiceName, startDate, endDate, customer, session);
                }
